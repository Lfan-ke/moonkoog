...
SECTIONS = [
    ("prompt", ("prompt/message.mbt", "prompt/prompt.mbt"), "Prompts and messages",
     "The conversation model every client speaks: typed messages (system, user, "
     "assistant, tool call, tool result) and the builder that assembles them into "
     "the Prompt an LLM request carries."),
    ("llm", ("llm/llm.mbt", "llm/openai_models.mbt", "llm/anthropic_models.mbt", "llm/google_models.mbt", "llm/deepseek_models.mbt", "llm/mistral_models.mbt"), "Models and the client contract",
     "LLModel and the LLMClient trait an executor implements, plus the catalogues "
     "of published OpenAI, Anthropic, Google, DeepSeek and Mistral models with "
     "their capabilities."),
    ("tools", ("tools/tool.mbt", "tools/prebuilt.mbt", "tools/file.mbt", "tools/shell.mbt", "tools/search.mbt"),
     "Tools",
     "A tool states its JSON schema through an explicit descriptor - MoonBit has no "
     "reflection to derive one from - and a ToolRegistry is what an agent is handed. "
     "The prebuilt file, shell and search tools are ready to register."),
    ("agent", ("agent/agent.mbt", "agent/strategy.mbt", "agent/events.mbt", "agent/client.mbt", "agent/endpoint.mbt"), "The agent",
     "AIAgent and the strategy graph it walks. run is not a special-cased loop, it "
     "is run_strategy wired with single_run_strategy. A RunHandle stops a run "
     "between nodes, so a tool call in progress always finishes. EventHandler "
     "observes every step."),
    ("strategies", ("agent/planner.mbt", "agent/critic.mbt", "agent/goap.mbt", "agent/choice.mbt", "agent/multi.mbt", "agent/structured.mbt"),
     "Strategies built on the graph",
     "Planning, self-criticism, GOAP, node choice, multi-agent delegation and "
     "schema-constrained output - each one a wiring of the same graph rather than a "
     "loop of its own."),
    ("resilience", ("agent/retry.mbt", "agent/caching.mbt", "agent/moderation.mbt", "agent/history.mbt", "agent/persistence.mbt", "agent/storage.mbt"),
     "Around the run",
     "Retry, response caching, moderation, history compression, checkpointing and "
     "the storage behind them."),
    ("client", ("client/openai.mbt", "client/anthropic.mbt", "client/embedder.mbt"), "LLM clients",
     "The OpenAI and Anthropic executors, and the embedding client. Native-only: "
     "they reach the network."),
    ("rag", ("embeddings/vector.mbt", "embeddings/embedder.mbt", "rag/vector_store.mbt", "rag/embedding_storage.mbt"),
     "Embeddings and retrieval",
     "Vectors and their similarity, the embedder contract, and the vector store a "
     "retrieval step queries."),
    ("memory", ("memory/memory.mbt", "persistence/persistence.mbt"), "Memory and persistence",
...
CONTRACT = """let agent = AIAgent::new(client, model=@llm.deepseek_v4_flash, tool_registry=tools)
...
def main():
    HEAD = ('<!doctype html><html lang="en"><head><meta charset="utf-8">'
            ...
            '<title>moonkoog — MoonBit agent framework API</title>'
    ...
    side = ['<aside class="sidebar"><div class="brand"><span class="dot"></span>moonkoog</div>'
            '<p class="brand-sub">MoonBit agent framework — API reference</p><nav class="side-nav">']
    ...
    side += ['</nav>'
             ...
             '<div class="side-foot">'
             '<a href="https://github.com/Lfan-ke/moonkoog/actions"><img alt="CI" src="https://img.shields.io/github/actions/workflow/status/Lfan-ke/moonkoog/ci.yml?branch=master&label=CI&logo=github"></a>'
             '<a href="https://mooncakes.io/docs/Lfan-ke/moonkoog"><img alt="mooncakes" src="https://img.shields.io/badge/mooncakes-Lfan--ke%2Fmoonorm-1f6feb"></a>'
             '</div></aside>']
    ...
    hero = ('<main><header class="hero"><h1>moonorm</h1>'
            ...
            '<a href="https://github.com/Lfan-ke/moonkoog"><img alt="GitHub" src="https://img.shields.io/badge/GitHub-source-24292f?logo=github"></a>'
            '<img alt="license" src="https://img.shields.io/badge/license-Apache--2.0-6d5efc"></div>'
            '<div class="install"><span class="prompt">$</span><code>moon add Lfan-ke/moonkoog</code>'
            '<button class="copy" data-copy="moon add Lfan-ke/moonkoog">copy</button></div>'
    ...
    body.append('<footer>Generated from source <code>///</code> doc-comments · '
                '<a href="https://mooncakes.io/docs/Lfan-ke/moonkoog">mooncakes</a> · '
                '<a href="https://github.com/Lfan-ke/moonkoog">GitHub</a> · Apache-2.0 &#169; Leo Cheng</footer>')
